...
class AssignmentTypeChecker {
    ...
    protected:
        ...
        struct Strategy {
            std::function<bool(const TypeSignature&, const TypeSignature&)> validate_assignment_all_intrnal; 
            std::function<bool(const TypeSignature&, const TemplateType&)> validate_assignment_generics_internal;
            static Strategy create_default_strategy(AssignmentTypeChecker& self); 
            static Strategy create_strict_strategy(AssignmentTypeChecker& self); 
        };
    ...
    private:
        ...
        [[nodiscard]] bool validate_assignment_very_strictly(
            const TypeSignature& source_generic, 
            const TypeSignature& dest_generic
        );
        ...
        std::shared_ptr<std::set<std::string>> strict_type_inference_deductions
            = std::make_shared<std::set<std::string>>();

        Strategy validation_strategy 
            = Strategy::create_default_strategy(*this);        
};